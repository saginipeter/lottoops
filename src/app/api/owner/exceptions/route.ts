...
export async function GET() {
  ...
  try {
    ...
    const exceptions = [
      ...locks.map((lock) => ({ id: lock.id, storeId: lock.storeId, type: "SEQUENCE_LOCK", store: storeNames.get(lock.storeId) ?? "Unknown store", game: lock.game.name, pack: lock.serialNumber, display: lock.slot?.slotNumber ?? "Unassigned", expected: lock.sequenceLockExpectedTicket, observed: lock.sequenceLockScannedTicket, timestamp: lock.sequenceLockedAt, status: resolutionsByStore.get(lock.storeId)?.has(`SEQUENCE_LOCK:${lock.id}`) ? "RESOLVED" : "UNRESOLVED" })),
      ...auditRows.map((row: { id: string; variance: number | null; endingExpectedTicket: number | null; endingPhysicalTicket: number | null; expectedTicket: number; slotNumber: string; pack: { serialNumber: string; game: { name: string } }; audit: { storeId: string; endedAt: Date | null; begunAt: Date } }) => ({ id: row.id, storeId: row.audit.storeId, type: "INVENTORY_AUDIT", store: storeNames.get(row.audit.storeId) ?? "Unknown store", game: row.pack.game.name, pack: row.pack.serialNumber, display: row.slotNumber, expected: row.endingExpectedTicket ?? row.expectedTicket, observed: row.endingPhysicalTicket, timestamp: row.audit.endedAt ?? row.audit.begunAt, status: resolutionsByStore.get(row.audit.storeId)?.has(`INVENTORY_AUDIT:${row.id}`) ? "RESOLVED" : "UNRESOLVED" })),
