...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const packs = await prisma.pack.findMany({ where: { storeId: { in: stores.map((store) => store.id) }, ...(status ? { status } : {}) }, select: { id: true, storeId: true, serialNumber: true, status: true, receivedAt: true, activatedAt: true, currentTicketNumber: true, ticketQuantity: true, game: { select: { gameNumber: true, name: true, price: true } }, slot: { select: { slotNumber: true } } }, orderBy: { receivedAt: "desc" }, take: 1000 }) as Array<{ id: string; storeId: string; serialNumber: string; status: string; receivedAt: Date; activatedAt: Date | null; currentTicketNumber: number | null; ticketQuantity: number | null; game: { gameNumber: string; name: string; price: unknown }; slot: { slotNumber: string } | null }>;
    const rows = packs.map((pack) => ({ store: storeNames.get(pack.storeId) ?? "Unknown store", gameNumber: pack.game.gameNumber, game: pack.game.name, serialNumber: pack.serialNumber, status: pack.status, display: pack.slot?.slotNumber ?? "", receivedAt: pack.receivedAt, activatedAt: pack.activatedAt, currentTicket: pack.currentTicketNumber, quantity: pack.ticketQuantity, price: Number(pack.game.price) }));
    if (request.nextUrl.searchParams.get("format") === "csv") {
      const escape = (value: unknown) => { const text = String(value ?? ""); return /[",\n]/.test(text) ? `"${text.replaceAll('"', '""')}"` : text; };
      const csv = [["Store", "Game #", "Game", "Pack", "Status", "Display", "Received", "Activated", "Current Ticket", "Quantity", "Price"], ...rows.map((row) => [row.store, row.gameNumber, row.game, row.serialNumber, row.status, row.display, row.receivedAt, row.activatedAt ?? "", row.currentTicket ?? "", row.quantity ?? "", row.price])].map((row) => row.map(escape).join(",")).join("\r\n");
