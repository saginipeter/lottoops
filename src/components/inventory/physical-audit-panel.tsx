...
export default function PhysicalAuditPanel({ shiftId, audit, activeDisplayPackCount }: { shiftId: string; audit?: AuditState; activeDisplayPackCount: number }) {
  ...
  // An empty display has nothing to reconcile. Treat 0/0 as complete so the
  // shift can move directly to closing without requiring a phantom scan.
  ...
  return (
    <Panel className="p-4">
      ...
      {audit && audit.status === "OPEN" && <>
        <div className="mt-3 flex items-center gap-2"><Button size="sm" variant={phase === "beginning" ? "secondary" : "ghost"} onClick={() => setPhase("beginning")}>Beginning</Button><Button size="sm" variant={phase === "ending" ? "secondary" : "ghost"} disabled={!beginningComplete} onClick={() => setPhase("ending")}>Ending</Button><span className={`text-sm font-medium ${scanned === activeDisplayPackCount ? "text-emerald-700" : "text-text-secondary"}`}>{scanned}/{activeDisplayPackCount} Scanned{scanned === activeDisplayPackCount ? " (Complete)" : ""}</span></div>
