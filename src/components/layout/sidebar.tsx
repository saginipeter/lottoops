...
const navSections: NavSection[] = [
  ...
  { label: "Inventory", items: [
    ...
    { href: "/inventory", label: "Back Stock", icon: Package },
...
const platformNavSections: NavSection[] = [{ label: "LottoOps Platform", items: [{ href: "/platform", label: "Control Center", icon: LayoutDashboard }] }];
const auditorNavSections: NavSection[] = [{ label: "Review", items: [
  { href: "/reports", label: "Reports", icon: BarChart3 },
  { href: "/documents", label: "Documents", icon: FileText },
  { href: "/support", label: "Support", icon: MessageCircle },
] }];

const roleLabel: Record<string, string> = { OWNER: "Owner", MANAGER: "Manager", SHIFT_LEAD: "Shift Lead", EMPLOYEE: "Employee", AUDITOR: "Auditor" };

interface SidebarProps {
  user: { name: string; role: string; storeName: string; storeNumber: string | null; storeAddress: string | null; storePhone: string | null; storeTimezone: string | null; initials: string; grantedPermissions: string[] };
}
...
export function Sidebar({ user }: SidebarProps) {
  ...
  const hasPermission = (permission?: string) => !permission || isManager || user.grantedPermissions.includes(permission);
  const sections = user.role === "PLATFORM_ADMIN" ? platformNavSections : user.role === "AUDITOR" ? auditorNavSections : isOwner ? ownerNavSections : navSections;

  useEffect(() => {
    const stored = window.localStorage.getItem("lottoops.sidebar.sections");
    if (stored) {
      try { setOpenSections(JSON.parse(stored) as Record<string, boolean>); } catch { /* reset malformed preference */ }
    }
  }, []);

  useEffect(() => {
    const activeSection = [...navSections, ...ownerNavSections, ...platformNavSections, ...auditorNavSections].find((section) => section.items.some((item) => {
  ...
  function toggleSection(label: string, isOpen: boolean) {
    ...
    window.localStorage.setItem("lottoops.sidebar.sections", JSON.stringify(next));
  }
  ...
  return <>
    ...
    <aside className={clsx("h-full shrink-0 flex-col bg-sidebar transition-[width] duration-200", collapsed ? "w-[72px]" : "w-[280px]", mobileOpen ? "fixed inset-y-0 left-0 z-50 flex w-[280px] shadow-2xl" : "hidden sm:flex")}>
      <div className={clsx("border-b border-sidebar-border py-5", collapsed ? "px-3" : "px-5")}>
        ...
        {!collapsed && <p className="mt-2 text-[10px] uppercase tracking-[0.2em] text-white/40">Store operations console</p>}
        {!collapsed && <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.07] p-3.5"><div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-accent/20 text-accent"><Building2 size={18} /></div><div className="min-w-0 flex-1"><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-white/40">Current location</p><p className="mt-1 truncate text-sm font-semibold text-white">{user.storeName}</p>{user.storeNumber && <p className="mt-0.5 text-[10px] text-white/50">Store {user.storeNumber}</p>}</div><ChevronDown size={15} className="mt-1 shrink-0 text-white/40" /></div>{storeDetails.length > 0 && <div className="mt-3 space-y-1.5 border-t border-white/10 pt-3 text-[10px] leading-4 text-white/55">{user.storeAddress && <p className="flex gap-2"><MapPin size={12} className="mt-0.5 shrink-0 text-accent/80" />{user.storeAddress}</p>}{user.storePhone && <p className="flex gap-2"><Phone size={12} className="mt-0.5 shrink-0 text-accent/80" />{user.storePhone}</p>}{user.storeTimezone && <p className="flex gap-2"><Clock size={12} className="mt-0.5 shrink-0 text-accent/80" />{user.storeTimezone}</p>}</div>}</div>}
      ...
      <nav className={clsx("flex-1 overflow-y-auto py-4", collapsed ? "px-2" : "px-4")} aria-label="Primary navigation">
        <Link href="/" onClick={() => setMobileOpen(false)} aria-current={pathname === "/" ? "page" : undefined} className={clsx("group flex min-h-10 items-center rounded-lg text-[13px] font-medium transition-colors", collapsed ? "justify-center px-2" : "gap-3 px-3", pathname === "/" ? "bg-accent text-white shadow-sm" : "text-sidebar-text hover:bg-sidebar-hover hover:text-sidebar-text-strong")}><LayoutDashboard size={17} strokeWidth={pathname === "/" ? 2.3 : 2} /><span className={clsx("truncate", collapsed && "hidden")}>Dashboard</span></Link>
        {sections.map((section) => {
          ...
          return <div key={section.label} className="mt-6 first:mt-5">
            ...
            {sectionIsOpen && visibleItems.map((item) => { const Icon = item.icon; const basePath = item.href.split("#")[0]; const isActive = pathname === basePath || pathname.startsWith(`${basePath}/`); return <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)} aria-current={isActive ? "page" : undefined} className={clsx("group flex min-h-10 items-center rounded-lg text-[13px] font-medium transition-colors", collapsed ? "justify-center px-2" : "gap-3 px-3", isActive ? "bg-accent text-white shadow-sm" : "text-sidebar-text hover:bg-sidebar-hover hover:text-sidebar-text-strong")}><Icon size={17} strokeWidth={isActive ? 2.3 : 2} className="shrink-0" /><span className={clsx("truncate", collapsed && "hidden")}>{item.label}</span>{item.badge && <span className="ml-auto rounded-full bg-white/15 px-2 py-0.5 text-[10px] text-white/70">{item.badge}</span>}{item.alert && <span className="ml-auto rounded-full bg-danger px-2 py-0.5 text-[10px] text-white">!</span>}</Link>; })}
          </div>;
        })}
      </nav>
      <div className={clsx("border-t border-sidebar-border", collapsed ? "p-2" : "p-4")}><div className={clsx("rounded-xl border border-white/10 bg-white/[0.05]", collapsed ? "p-2" : "p-3")}><div className="flex items-center gap-2.5"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-accent text-[11px] font-semibold text-white">{user.initials}</div><div className={clsx("min-w-0 flex-1", collapsed && "hidden")}><p className="truncate text-xs font-semibold text-white/90">{user.name}</p><p className="text-[10px] uppercase tracking-wider text-white/40">{roleLabel[user.role] ?? user.role}</p></div></div><div className={clsx("mt-3 border-t border-white/10 pt-2", collapsed && "hidden")}><LogoutButton /></div></div></div>
    </aside>
