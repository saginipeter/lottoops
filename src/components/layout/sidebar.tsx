...
export function Sidebar({ user }: SidebarProps) {
  ...
  return <>
    ...
    <aside className={clsx("h-full shrink-0 flex-col border-r border-sidebar-line bg-sidebar-deep transition-[width] duration-200", collapsed ? "w-[72px]" : "w-[280px]", mobileOpen ? "fixed inset-y-0 left-0 z-50 flex w-[280px] shadow-2xl" : "hidden sm:flex")}>
      <div className={clsx("border-b border-sidebar-line py-5", collapsed ? "px-3" : "px-5")}>
        ...
        {!collapsed && <div className="mt-5 border border-sidebar-line bg-sidebar-panel p-3.5"><div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center border border-accent/40 bg-accent/15 text-accent"><Building2 size={18} /></div><div className="min-w-0 flex-1"><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-white/40">Current location</p><p className="mt-1 truncate text-sm font-semibold text-white">{user.storeName}</p>{user.storeNumber && <p className="mt-0.5 text-[10px] text-white/50">Store {user.storeNumber}</p>}</div><ChevronDown size={15} className="mt-1 shrink-0 text-white/40" /></div>{storeDetails.length > 0 && <div className="mt-3 space-y-1.5 border-t border-sidebar-line pt-3 text-[10px] leading-4 text-white/55">{user.storeAddress && <p className="flex gap-2"><MapPin size={12} className="mt-0.5 shrink-0 text-accent/80" />{user.storeAddress}</p>}{user.storePhone && <p className="flex gap-2"><Phone size={12} className="mt-0.5 shrink-0 text-accent/80" />{user.storePhone}</p>}{user.storeTimezone && <p className="flex gap-2"><Clock size={12} className="mt-0.5 shrink-0 text-accent/80" />{user.storeTimezone}</p>}</div>}</div>}
      ...
      <div className={clsx("border-t border-sidebar-line", collapsed ? "p-2" : "p-4")}><div className={clsx("border border-sidebar-line bg-sidebar-panel", collapsed ? "p-2" : "p-3")}><div className="flex items-center gap-2.5"><div className="flex h-9 w-9 shrink-0 items-center justify-center bg-accent text-[11px] font-semibold text-white">{user.initials}</div><div className={clsx("min-w-0 flex-1", collapsed && "hidden")}><p className="truncate text-xs font-semibold text-white/90">{user.name}</p><p className="text-[10px] uppercase tracking-wider text-white/40">{roleLabel[user.role] ?? user.role}</p></div></div><div className={clsx("mt-3 border-t border-sidebar-line pt-2", collapsed && "hidden")}><LogoutButton /></div></div></div>
    </aside>
