...
export function Sidebar({ user }: SidebarProps) {
  ...
  return <>
    ...
    <aside className={clsx("h-full shrink-0 flex-col border-r border-[#3b4770] bg-[#211943] transition-[width] duration-200", collapsed ? "w-[72px]" : "w-[280px]", mobileOpen ? "fixed inset-y-0 left-0 z-50 flex w-[280px] shadow-2xl" : "hidden sm:flex")}>
      <div className={clsx("border-b border-[#3b4770] py-5", collapsed ? "px-3" : "px-5")}>
      ...
      <nav className={clsx("flex-1 overflow-y-auto py-4", collapsed ? "px-2" : "px-4")} aria-label="Primary navigation">
        <Link href="/" onClick={() => setMobileOpen(false)} aria-current={pathname === "/" ? "page" : undefined} className={clsx("group flex min-h-10 items-center border-l-2 text-[13px] font-medium transition-colors", collapsed ? "justify-center border-transparent px-2" : "gap-3 px-3", pathname === "/" ? "border-accent bg-[#35275f] text-white" : "border-transparent text-sidebar-text hover:bg-[#302653] hover:text-sidebar-text-strong")}><LayoutDashboard size={17} strokeWidth={pathname === "/" ? 2.3 : 2} /><span className={clsx("truncate", collapsed && "hidden")}>Dashboard</span></Link>
