...
export function InvoiceStep({ shipment, setShipment, nextStep, onCancel }: InvoiceStepProps) {
  ...
  return <Panel className="p-4 sm:p-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <Field label="Expected invoice total" icon={<Package size={15} />}><div className="relative"><span className="absolute left-3 top-3 text-sm text-text-tertiary">$</span><input type="number" inputMode="decimal" min="0" step="0.01" className="h-11 w-full border border-border bg-muted/20 pl-7 pr-3 text-sm outline-none focus:border-accent" value={shipment.expectedRetailValue ?? ""} onChange={(e) => setShipment((prev) => ({ ...prev, expectedRetailValue: e.target.value === "" ? undefined : Number(e.target.value) }))} /></div></Field>
    </div>
