...
export function EmployeeDeviceSetup({ terminalId }: { terminalId: string }) {
  ...
  return <section className="mt-3 border border-[#cbd5e1] bg-white p-4 shadow-[0_6px_16px_rgba(23,35,63,0.04)]" aria-label="Device setup">
    <div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center border border-[#cbd5e1] bg-[#eef2f7] text-[#314a8a]"><Smartphone size={20} /></div><div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#64748b]">Device setup</p><h2 className="text-base font-bold text-[#17233f]">Install and authorize this device</h2><p className="mt-1 text-xs leading-5 text-[#64748b]">Install LottoOps for quick access, then enter the six-digit code supplied by your manager.</p></div></div>
    <div className="mt-4 grid grid-cols-1 gap-2 sm:flex sm:flex-wrap"><button type="button" onClick={() => { void install(); }} disabled={installed} className="inline-flex min-h-12 items-center justify-center gap-2 border border-[#cbd5e1] bg-[#f8fafc] px-4 text-sm font-bold text-[#17233f] disabled:opacity-60"><Download size={16} />{installed ? "App installed" : installEvent ? "Install app" : "How to install"}</button>{paired && <span className="inline-flex min-h-12 items-center justify-center gap-2 bg-[#dcfce7] px-4 text-sm font-bold text-[#166534]"><KeyRound size={16} />Paired</span>}</div>
    ...
    {!paired && <div className="mt-3 grid grid-cols-[1fr_auto] gap-2"><input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" placeholder="6-digit code" className="min-h-12 min-w-0 border border-[#cbd5e1] bg-white px-3 text-base font-mono tracking-[0.2em] outline-none focus:border-[#314a8a] focus:ring-2 focus:ring-[#314a8a]/20" /><button type="button" onClick={() => { void pair(); }} disabled={saving || code.length !== 6} className="inline-flex min-h-12 items-center justify-center gap-2 bg-[#314a8a] px-4 text-sm font-bold text-white disabled:opacity-50"><RefreshCw size={15} className={saving ? "animate-spin" : ""} />Pair</button></div>}
